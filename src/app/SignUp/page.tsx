...
import { Droplets, Heart, ShieldCheck, Zap, MapPin, Users } from "lucide-react";

const STEPS = [
  { icon: ShieldCheck, title: "Create your profile",  body: "Fill in your blood group, area, and availability. Takes under 2 minutes." },
  { icon: MapPin,      title: "Get found nearby",     body: "People searching in your area will see your profile when they need your blood type." },
  { icon: Heart,       title: "Make contact",         body: "They reach out directly. You decide whether and when to donate." },
];
...
const RegisterPage = () => {
  return (
    <main className="h-screen w-full overflow-hidden lg:flex">
      ...
      ══════════════════════════════════════════════════════ */}
      <div
        className="relative hidden h-full w-[52%] shrink-0 overflow-hidden lg:flex lg:flex-col"
      ...
      >
        ...
        <div className="pointer-events-none absolute left-1/2 top-1/2 h-[700px] w-[700px] -translate-x-1/2 -translate-y-1/2 rounded-full"
          style={{ background:"radial-gradient(circle,rgba(226,72,77,0.20) 0%,transparent 65%)" }} />
        <div className="pointer-events-none absolute right-[-80px] top-[-60px] h-[400px] w-[400px] rounded-full"
          style={{ background:"radial-gradient(circle,rgba(226,72,77,0.13) 0%,transparent 65%)" }} />
        <div className="pointer-events-none absolute bottom-[-60px] left-[-30px] h-[340px] w-[340px] rounded-full"
          style={{ background:"radial-gradient(circle,rgba(185,47,53,0.17) 0%,transparent 65%)" }} />

        {/* cross-hatch */}
        <svg className="pointer-events-none absolute inset-0 h-full w-full" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="cross-l2" x="0" y="0" width="32" height="32" patternUnits="userSpaceOnUse">
              <line x1="0" y1="0" x2="32" y2="32" stroke="rgba(226,72,77,0.055)" strokeWidth="0.5"/>
              <line x1="32" y1="0" x2="0" y2="32" stroke="rgba(226,72,77,0.055)" strokeWidth="0.5"/>
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#cross-l2)" />
        </svg>

        {/* ghost drops */}
        <svg className="pointer-events-none absolute right-0 top-[-20px] opacity-[0.055]" width="280" height="340" viewBox="0 0 280 340" fill="none">
          <path d="M140 16C140 16 16 140 16 215C16 281 72 326 140 326C208 326 264 281 264 215C264 140 140 16 140 16Z" fill="#E2484D"/>
        </svg>
        <svg className="pointer-events-none absolute bottom-[-10px] left-[-20px] opacity-[0.045]" width="200" height="240" viewBox="0 0 200 240" fill="none">
          <path d="M100 12C100 12 12 100 12 155C12 202 52 232 100 232C148 232 188 202 188 155C188 100 100 12 100 12Z" fill="#E2484D"/>
        </svg>
        <svg className="pointer-events-none absolute right-8 top-[42%] opacity-[0.038]" width="100" height="122" viewBox="0 0 100 122" fill="none">
        ...
        <div className="relative z-10 flex h-full flex-col justify-between p-10 xl:p-12">
          ...
          <div className="flex flex-col gap-8">
            <div>
              <p className="mb-3 text-[11px] font-bold tracking-[0.18em] text-[#E2484D]/80">JOIN THE COMMUNITY</p>
              <h2 className="text-[44px] font-extrabold leading-[1.04] tracking-[-0.04em] text-white xl:text-[50px]">
                Become a<br />
                <span style={{ background:"linear-gradient(90deg,#E2484D 0%,#FF8285 100%)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>
                  donor.
                </span>
              </h2>
              <p className="mt-3 max-w-[300px] text-[14px] leading-[1.75] text-white/45">
                Keep your profile updated so people nearby can find you when they need blood most.
              </p>
            </div>

            <div className="flex flex-col gap-3">
              <p className="text-[11px] font-semibold text-white/30">HOW IT WORKS</p>
              {STEPS.map(({ icon: Icon, title, body }, i) => (
                <div key={title} className="flex gap-4 rounded-[16px] px-4 py-4"
                  style={{ background:"rgba(255,255,255,0.055)", border:"1px solid rgba(255,255,255,0.08)", backdropFilter:"blur(10px)" }}>
                  <div className="flex flex-col items-center gap-1.5 pt-0.5">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#E2484D]/20">
                      <Icon size={14} strokeWidth={2} className="text-[#E2484D]" />
                    </div>
                    <span className="text-[10px] font-bold text-[#E2484D]/50">0{i+1}</span>
                  </div>
                  <div className="min-w-0">
                    <p className="text-[13.5px] font-semibold text-white">{title}</p>
                    <p className="mt-0.5 text-[12px] leading-[1.65] text-white/40">{body}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* ══════════════════════════════════════════════════════
          RIGHT — Light form panel  (rich background)
      ══════════════════════════════════════════════════════ */}
      <div
      ...
      >
        ...
        <svg className="pointer-events-none absolute inset-0 h-full w-full" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="dots-r2" x="0" y="0" width="28" height="28" patternUnits="userSpaceOnUse">
              <circle cx="1.5" cy="1.5" r="1" fill="rgba(226,72,77,0.10)" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#dots-r2)" />
        </svg>
        ...
        <svg className="pointer-events-none absolute right-10 top-[12%] opacity-[0.045]" width="80" height="98" viewBox="0 0 80 98" fill="none">
          <path d="M40 5C40 5 5 40 5 62C5 81 21 93 40 93C59 93 75 81 75 62C75 40 40 5 40 5Z" fill="#E2484D"/>
        </svg>
        <svg className="pointer-events-none absolute bottom-[16%] left-8 opacity-[0.04]" width="60" height="74" viewBox="0 0 60 74" fill="none">
          <path d="M30 4C30 4 4 30 4 46C4 60 15 70 30 70C45 70 56 60 56 46C56 30 30 4 30 4Z" fill="#E2484D"/>
        </svg>

        {/* decorative rings */}
        <div className="pointer-events-none absolute left-1/2 top-1/2 h-[540px] w-[540px] -translate-x-1/2 -translate-y-1/2 rounded-full"
          style={{ border:"1px solid rgba(226,72,77,0.07)" }} />
        <div className="pointer-events-none absolute left-1/2 top-1/2 h-[660px] w-[660px] -translate-x-1/2 -translate-y-1/2 rounded-full"
        ...
        {/* Glass form card */}
        <div
          className="relative z-10 w-full max-w-[460px] rounded-[24px] p-8"
          style={{
